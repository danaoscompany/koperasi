...
$this->upload->initialize($config);
    if ($this->upload->do_upload('file')) {
      ...
      if ($kodeProject == 'SYPG-01-002') {
        ...
        $simpananWajib = $jumlah;
          // Insert Simpanan Wajib
          // Get last topup date
          $query2 = $this->db->query("SELECT * FROM riwayat_simpanan WHERE no_anggota='" . $noAnggota . "' AND kode_project='SYPG-01-003' ORDER BY synced_at DESC LIMIT 1")->result_array();
          if (sizeof($query2) > 0) {
            $tanggal = $query2[0]['synced_at'];
            $tanggal = date('Y:m:d H:i:s', strtotime('+1 month', strtotime($tanggal)));
          }
  ...
  public function get_mutlaqoh_value($userID) {
      ...
      $total = 0;
      $query = $this->db->query("SELECT * FROM riwayat_simpanan WHERE no_anggota='" . $noAnggota . "' AND (kode_project='SYPG-01-002' OR kode_project='SYPG-01-003') AND paid=1")->result_array();
      for ($i=0; $i<sizeof($query); $i++) {
          $total += intval($query[$i]['debet']);
      }
      return $total;
  ...
  public function get_syirkah_muqoyyad() {
  	  $userID = intval($this->get_post_value('user_id'));
  	  $noAnggota = $this->db->get_where('nasabah', array(
          'user_id' => $userID
      ))->row_array()['no_anggota'];
      $query = $this->db->query("SELECT * FROM riwayat_simpanan WHERE no_anggota='" . $noAnggota . "' AND kode_project <> 'SYPG-01-002' AND kode_project <> 'SYPG-01-003'")->result_array();
      $total = 0;
      for ($i=0; $i<sizeof($query); $i++) {
      	  $total += intval($query[$i]['debet']);
      }
  ...
  public function get_riwayat_tabungan_2() {
    $userID = intval($this->get_post_value('user_id'));
  ...
  public function get_modal_history() {
      ...
      $query = $this->db->query("SELECT * FROM riwayat_simpanan WHERE no_anggota='" . $noAnggota . "' AND kode_project <> 'SYPG-01-005' ORDER BY synced_at ASC")->result_array();
      for ($i=0; $i<sizeof($query); $i++) {
          $query[$i]['nama_project'] = $this->db->get_where('project', array(
